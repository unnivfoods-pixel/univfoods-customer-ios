...
async function seedData() {
    ...
    const legalDoc = {
        ...
        content: 'Welcome to UNIV. By using our service, you agree to... (Full Terms would go here)',
        version: '1.0',
        is_active: true,
        published_at: new Date().toISOString()
    };

    console.log(">>> Inserting Legal Doc...");
    const { error: legalError } = await supabase.from('legal_documents').upsert(legalDoc, { onConflict: 'type' });
    if (legalError) console.error("Legal Error:", legalError.message);

    // 3. Seed System Settings
    const settings = [
        { key: 'support_phone', value: '+919940407600', description: 'Primary support contact' },
        { key: 'support_email', value: 'support@univfoods.in', description: 'Primary support email' },
        { key: 'emergency_phone', value: '100', description: 'Local emergency services' }
    ];

    console.log(">>> Inserting System Settings...");
    const { error: settingsError } = await supabase.from('system_settings').upsert(settings, { onConflict: 'key' });
