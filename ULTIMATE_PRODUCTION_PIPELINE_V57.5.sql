...
-- NUCLEAR IDENTITY & LOGISTICS RESTORATION V57.5
...
-- Check if vendors table exists before altering
DO $$ BEGIN
...
BEGIN
    ...
    IF p_role = 'vendor' THEN
        -- AGGRESSIVE CLAIMER: If this user doesn't own ANY vendor, give them the first unowned one.
        -- This prevents the "Blank App" issue for vendors.
        IF NOT EXISTS (SELECT 1 FROM public.vendors WHERE owner_id = p_user_id) THEN
            UPDATE public.vendors 
            SET owner_id = p_user_id 
            WHERE id = (SELECT id FROM public.vendors WHERE owner_id IS NULL OR owner_id = '' LIMIT 1);
        END IF;

        SELECT row_to_json(v)::jsonb INTO v_profile FROM public.vendors v WHERE owner_id = p_user_id LIMIT 1;
        
        IF v_profile IS NOT NULL THEN
            v_vendor_id := v_profile->>'id';
            
            -- Load Vendor Products (Menu)
            SELECT json_agg(p)::jsonb INTO v_menu FROM public.products p WHERE vendor_id::TEXT = v_vendor_id;
            ...
            -- Load Stats (Calculated from view)
            SELECT row_to_json(s)::jsonb INTO v_stats FROM public.vendor_earnings_stats s WHERE vendor_id::TEXT = v_vendor_id;
            
            -- Fallback for stats if no record in view yet
            IF v_stats IS NULL THEN
                v_stats := jsonb_build_object('total_earnings', 0, 'pending_orders', 0, 'completed_orders', 0);
            END IF;
        END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'stats', COALESCE(v_stats, '{"total_earnings":0,"pending_orders":0,"completed_orders":0}'::jsonb),
        'timestamp', NOW()
    );
END;
