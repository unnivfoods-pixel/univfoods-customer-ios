-- 🛰️ COMPLETE REPAIR V2: RIDER NAME + 15KM DISTANCE
-- This script fixes the column error by dropping the view first and then recreating it.

BEGIN;

-- 🛡️ 1. DROP EXISTING VIEW TO AVOID "column name" ERRORS
-- Postgres does not allow changing column names/order with CREATE OR REPLACE VIEW.
DROP VIEW IF EXISTS public.order_tracking_stabilized_v1;

-- 🛡️ 2. CREATE STABILIZED VIEW
-- Maps "name" correctly so you don't get the "full_name" error anymore.
...
SELECT 
    ...
    r.name AS rider_name, -- 🛡️ FIXED COLUMN NAME HERE (r.name instead of r.full_name)
...
COMMIT;
SELECT 'DATABASE STABILIZED V2: VIEW RECREATED & DISTANCE FIXED' as status;
