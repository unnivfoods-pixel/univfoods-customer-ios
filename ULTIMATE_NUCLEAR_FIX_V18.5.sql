-- 🔥 ULTIMATE NUCLEAR FIX (V18.5 - Legacy ID Cleanup & Migration)
-- Purpose: Force orders to UUID by purging invalid non-UUID legacy IDs ("ORD-XXXX").
...
-- 2. PURGE INVALID IDs: Delete orders that aren't valid UUID strings
-- This stops the "invalid input syntax for type uuid" error.
DELETE FROM public.orders 
WHERE id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';

-- 3. CLEANUP DUPLICATES: Keep only one of each ID if any exist
DELETE FROM public.orders a USING public.orders b 
WHERE a.ctid < b.ctid AND a.id = b.id;

-- 4. FORCE DATA TYPE: Now that only valid strings remain, we cast to UUID
...
-- 6. RECREATE VIEW: order_details_v3
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Skipping some tables for publication.';
END $$;
