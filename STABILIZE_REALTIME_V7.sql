-- 🛰️ REALTIME STABILIZER V7: FIXED RLS + ENABLE BROADCAST
-- This script fixes the 'column "metadata" does not exist' error.
-- It ensures the 'orders' table broadcasts updates and that users have permission to receive them.
...
-- We use a simpler, safer policy that matches the customer_id directly.
...
-- Policy for standard Supabase Auth users
CREATE POLICY "Allow individual read access" 
ON public.orders FOR SELECT 
USING (auth.uid()::text = customer_id);

-- Policy for ANON/GUEST access (required for the app's current setup)
-- ⚠️ In a production app, you would restrict this more, but we enable it here to ensure updates work.
CREATE POLICY "Allow anon read access" 
ON public.orders FOR SELECT 
TO anon
USING (customer_id IS NOT NULL);

-- 🛡️ 4. RE-SYNC VIEW WITH LATEST REFINEMENTS
...
SELECT 'DATABASE STABILIZED V7: REALTIME FIXED' as status;
