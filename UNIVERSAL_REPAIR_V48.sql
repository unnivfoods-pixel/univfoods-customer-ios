-- UNIVERSAL REPAIR v48.0 (Orders Visibility Fix)
-- 🎯 MISSION: Fix "Orders not displaying" and missing tracking view.
...
-- 🛠️ 2. SHARED ORDER VIEW (Rich data for UI)
...
-- 🛠️ 3. UI EXPECTATION SYNC (The "tracking" view the OrderStore wants)
CREATE OR REPLACE VIEW public.order_tracking_details_v1 AS
SELECT * FROM public.order_details_v3;

-- 🛠️ 4. IDENTITY UNIFICATION (The Atomic Fix)
-- Ensure both are TEXT to prevent matching failures
...
-- 🛠️ 6. FIX UPDATES (Ensure vendors are really ONLINE)
UPDATE public.vendors SET status = 'ONLINE', is_active = TRUE, is_approved = TRUE, is_open = TRUE;

COMMIT;
SELECT 'NUCLEAR REPAIR V48 COMPLETE' as status;
