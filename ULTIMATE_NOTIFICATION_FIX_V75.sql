-- 🔔 ULTIMATE NOTIFICATION FIX v75.0 (AGGRESSIVE RLS FIX)
...
-- ⚠️ DROP ALL POLICIES FIRST (They prevent column type changes)
DO $$
DECLARE
    pol_record RECORD;
BEGIN
    ...
    LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(pol_record.policyname) || ' ON public.notifications';
    END LOOP;
END $$;

-- Also drop specific known ones just in case
DROP POLICY IF EXISTS "Users can see their own notifications" ON public.notifications;
DROP POLICY IF EXISTS "Allow all access" ON public.notifications;
DROP POLICY IF EXISTS "IsolationV3_Notifications" ON public.notifications;
...
-- Optional: Add an admin-all-access policy if needed
-- CREATE POLICY "Admins have full access" ON public.notifications FOR ALL USING (true);
...
CREATE OR REPLACE FUNCTION public.fn_unified_notification_v75()
RETURNS TRIGGER AS $$
DECLARE
    v_cust_id TEXT;
    v_vend_id TEXT;
...
CREATE TRIGGER tr_unified_notifications_v75
AFTER INSERT OR UPDATE ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.fn_unified_notification_v75();
...
CREATE OR REPLACE FUNCTION public.mark_notif_read_v75(p_notif_id UUID)
RETURNS VOID AS $$
BEGIN
...
SELECT '✅ NOTIFICATION ENGINE v75.0 INSTALLED SUCCESSFULLY!' as status;
